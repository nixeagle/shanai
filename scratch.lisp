...
(defun read-tier-node (in)
  (let ((level (read-u1 in)))
    (multiple-value-bind (string length) (shanai.binary-data:read-qtstring in)
...
(defun read-player-info (s)
  (values (list (read-u4 s)
                (shanai.binary-data:read-qtstring s)
                (shanai.binary-data:read-qtstring s)
                (read-u1 s)
...
(define-po-protocol-reader send-message 4 (in)
  ...
  (shanai.binary-data:read-qtstring in))
(flet ((puid (stream)
         (cons (read-u2 stream) (read-u1 stream))))
  (define-po-protocol-reader player-list 5 (in)
    (list :player-id (read-u4 in)
          :player-name (shanai.binary-data:read-qtstring in)
          :player-info (shanai.binary-data:read-qtstring in)
          ...
          :player-tier (shanai.binary-data:read-qtstring in)
...
(defun read-poke-battle-data (in)
  (let ((pnum (read-u2 in))
        (forme (read-u1 in))
        (nick (shanai.binary-data:read-qtstring in))
        (max-hp (read-u2 in)))
    (let ((db-poke (gethash pnum pokemon::*pokedex*)))
      (make-instance 'shanai.pokemon:battle-pokemon
                     :id pnum
                     :type (pokemon-type (gethash pnum pokemon::*pokedex*))
                     :forme forme
                     :nickname nick
                     ...
                     :ivs (make-stats (read-u4 in) (read-u4 in) (read-u4 in)
                                      (read-u4 in) (read-u4 in) (read-u4 in))
                     :type (list (generic:type1 db-poke)
                                 (generic:type2 db-poke))))))
...
(defun write-ask-for-pass (hash out)
  ...
  (write-u1 out 13)
  (shanai.binary-data:write-qtstring hash out))
...
(define-po-protocol-reader channels-list 44 (in)
  (loop for i from (read-u4 in) downto 1
     collecting (cons (read-u4 in) (shanai.binary-data:read-qtstring in))))
...
That is the qtstring comes before the channel-id."
  (let ((name (shanai.binary-data:read-qtstring in)))
    (list :channel-id (read-u4 in)
          :channel-name name)))
...
Most common usage is for whenever a user does /me."
  (list :channel-id (read-u4 in)
        :channel-html (shanai.binary-data:read-qtstring in)))
...
(define-po-battle-protocol-reader send-out 0 (in)
  ...
  (list :to-spot (read-u1 in)
        ...
        :forme-id (read-u1 in)
        :pokemon-nick (shanai.binary-data:read-qtstring in)
...
(define-po-battle-protocol-reader stat-change 12 (in)
  (list :stat-index (read-u1 in)
        :stat-value (shanai.binary-data:read-s1  in)))
...
(define-po-battle-protocol-reader battle-chat 16 (in)
  ...
  (list :message (shanai.binary-data:read-qtstring in)))
...
effects." 
  (list :attack (shanai.binary-data:read-s1  in) :defense (shanai.binary-data:read-s1  in)
        :special-attack (shanai.binary-data:read-s1  in) :special-defense (shanai.binary-data:read-s1  in)
        :speed (shanai.binary-data:read-s1  in) :unknown1 (shanai.binary-data:read-s1  in)
        :unknown2 (shanai.binary-data:read-s1  in) :unknown3 (shanai.binary-data:read-s1  in)))
...
(define-po-battle-protocol-reader tier-section 40 (in)
  "Is the battle rated or not?"
  (list (shanai.binary-data:read-qtstring in)))
...
(defun write-change-team (name stream &key
  ...
  (let ((out-vector (with-output-to-vector (s nil :external-format :utf-16be)
                      (write-u1 s 6)
                      (shanai.binary-data:write-qtstring nickname s)
                      (shanai.binary-data:write-qtstring info s)
                      (shanai.binary-data:write-qtstring lose s)
                      (shanai.binary-data:write-qtstring win s)
                      (write-u2 s avatar)
                      (shanai.binary-data:write-qtstring default-tier s)
...
(defun %write-poke-personal (pokemon stream
  ...
  (shanai.binary-data:write-qtstring nickname stream)
...
(defun %locate-trait-position-hack (trait-string)
  (cond ((string= "Own Tempo" trait-string) 20)
        ...
        ((string= "Unburden" trait-string) 84)
        ((string= "Compoundeyes" trait-string) 14)
        ((string= "Limber" trait-string) 7)
        ((string= "Iron Fist" trait-string) 89)
